...
function TreeSet(root: Tree): set<Tree>
{
  match root {
    ...
    case Node(val, left, right) =>
      {root} + TreeSet(left) + TreeSet(right)
  }
}
  ...
  ensures TreeHeight(root) >= -1
{
  if root == Nil then
    ...
    max(TreeHeight(root.left), TreeHeight(root.right)) + 1
}
...
predicate isPath(paths: seq<Tree>)
{
  if |paths| == 0 then
    ...
    match paths[0] { case Nil => false case Node(val, left, right) => (left == paths[1] || right == paths[1] || (paths[1] != Nil && (paths[0] == paths[1].left || paths[0] == paths[1].right))) && isPath(paths[1..]) }
}

predicate isValidTreePath(path: seq<Tree>, start: Tree, end: Tree)
{
  isTreePath(path, start, end) &&
  exists root: Tree :: 
    root != Nil &&
    root in path &&
    forall node: Tree :: 
      node in path ==>
        node in TreeSet(root)
}
...
predicate isTreePath(path: seq<Tree>, start: Tree, end: Tree)
{
  if |path| == 0 then
    ...
    match path[0] { case Nil => false case Node(val, left, right) => path[0] == start && path[|path| - 1] == end && isParentOrChild(path[0], path[1]) && isTreePath(path[1..], path[1], end) }
}
...
predicate isTreePathAlt(path: seq<Tree>, start: Tree, end: Tree)
{
  if |path| == 0 then
    ...
    path[0] == start && start != Nil && end == path[|path| - 1] == end && forall i :: 0 <= i < |path| - 1 ==> isParentOrChild(path[i], path[i + 1])
}
...
predicate isDescTreePath(path: seq<Tree>, start: Tree, end: Tree)
{
  if |path| == 0 then
    ...
    match path[0] { case Nil => false case Node(val, left, right) => end != Nil && path[|path| - 1] == end && path[0] == start && path[1] != Nil && ((left == path[1] && isDescTreePath(path[1..], left, end)) || (right == path[1] && isDescTreePath(path[1..], right, end))) }
}
...
predicate isAscTreePath(paths: seq<Tree>, start: Tree, end: Tree)
{
  if |paths| == 0 then
    ...
    match paths[0] { case Nil => false case Node(val, left, right) => end != Nil && paths[|paths| - 1] == end && start == paths[0] && paths[1] != Nil && ((paths[0] == paths[1].left && isAscTreePath(paths[1..], paths[1], end)) || (paths[0] == paths[1].right && isAscTreePath(paths[1..], paths[1], end))) }
}
...
predicate isAscPath(paths: seq<Tree>, root: Tree)
{
  if |paths| == 0 then
    ...
    match paths[0] { case Nil => false case Node(val, left, right) => root == paths[0] && paths[1] != Nil && isAscPath(paths[1..], paths[1]) && (paths[0] == paths[1].left || paths[0] == paths[1].right) }
}
...
predicate isDescPath(paths: seq<Tree>, root: Tree)
{
  if |paths| == 0 then
    ...
    match paths[0] { case Nil => false case Node(val, left, right) => paths[0] == root && ((left == paths[1] && isDescPath(paths[1..], left)) || (right == paths[1] && isDescPath(paths[1..], right))) }
}
  ...
  ensures forall x :: x in path[1..] ==> x in TreeSet(root) - {root}
{
  if |path| == 2 {
    ...
    } else if isDescTreePath(path[1..], root.right, end) <== root.right != Nil {
      parentNotInTreeSet(root, root.right);
      forall x | x in path[1..]
        ensures x in TreeSet(root) - {root}
      {
