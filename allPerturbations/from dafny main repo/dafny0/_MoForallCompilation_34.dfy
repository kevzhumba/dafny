...
method TestAggregateMultiArrayUpdate()
{
  ...
  PrintMatrix(matrix);
  var m' := new int[matrix.Length1, matrix.Length0] ((i, j) requires 0 <= i < matrix.Length1 && 0 <= j < matrix.Length0 reads matrix => matrix[j, i]);
...
method Transpose<T(0)>(m: array2<T>)
    returns (m0: array2<T>, m1: array2<T>, m2: array2<T>)
