...
module PriorityQueue_layered {
  ...
  function Min(t: T): int
    ...
    ensures var m := Min(t); m in Elements(t) && forall x :: x in Elements(t) ==> m <= x
  {
    PQ.AboutMin(t);
    PQ.Min(t)
  }
  ...
  type T = t: PQ.T
    | PQ.Valid(t)
    witness (PQ.AboutEmpty(); PQ.Empty())
}
...
module PriorityQueue_intrinsic {
  ...
  function Empty(): T
    ensures var t' := Empty(); Valid(t') && Elements(t') == multiset{}
  {
    PQ.AboutEmpty();
    PQ.Empty()
  }
  ...
  function Insert(t: T, x: int): T
    requires Valid(t)
    ensures var t' := Insert(t, x); Valid(t') && Elements(t') == Elements(t) + multiset{x}
  {
    PQ.AboutInsert(t, x);
    PQ.Insert(t, x)
  }
...
module PriorityQueue_extrinsic {
  ...
  export
    provides T, Valid, Elements, Empty, Insert, Min, AboutEmpty, AboutInsert, AboutMin
...
module PriorityQueue_direct {
  ...
  lemma AboutMin(t: T)
    requires t != Leaf
    ensures t.val in Elements(t) && forall x :: x in Elements(t) ==> t.val <= x
  {
  }
