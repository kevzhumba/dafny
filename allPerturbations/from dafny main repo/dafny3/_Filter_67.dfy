...
lemma Lemma_TailSubStreamK(s: Stream, u: Stream, k: nat)
  ...
  ensures forall i :: 0 <= i < NextMinimizer(s, P, n) ==> !P(Tail(s, i).head)
{
  if forall i :: 0 <= i < n ==> !P(Tail(s, i).head) then
    ...
    var k :| 0 <= k < n && P(Tail(s, k).head); NextMinimizer(s, P, k)
}
  ...
  decreases k
{
  ...
  if s.head == x {
    ...
    assert fs == Cons(s.head, Filter(s.tail, P));
    calc {
      true;
    ==
      {
    ...
    assert fs == Filter(s.tail, P);
  }
}

greatest predicate Increasing(s: Stream, ord: Ord)
{
  ord(s.head) < ord(s.tail.head) &&
  Increasing(s.tail, ord)
}

greatest predicate IncrFrom[nat](s: Stream, low: int, ord: Ord)
{
  low <= ord(s.head) &&
  IncrFrom(s.tail, ord(s.head) + 1, ord)
}
