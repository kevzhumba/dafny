...
class UnboundedStack<T> {
    ...
    reads this, representation
  {
    ...
    (top == null ==>
      content == []) &&
    (top != null ==>
      top in representation &&
      this !in top.footprint &&
      top.footprint <= representation &&
      content == top.content &&
      top.Valid())
  }
...
class Node<T> {
  ...
  constructor InitNode(val: T, next: Node?<T>)
    ...
    ensures Valid()
    ensures next != null ==> content == [val] + next.content && footprint == {this} + next.footprint
    ensures next == null ==> content == [val] && footprint == {this}
  {
    this.val := val;
    this.next := next;
    if next == null {
